...
test('POST /api/events returns 503 when dependency fails', async ({ request }) => {
  ...
  const res = await request.post(url, {
    headers: { 'x-simulate-db-down': 'true' },
    data: {
      tenantId: 'tenant-1',
      eventId,
      actor: 'aj',
  ...
  expect(['dependency_failure', 'circuit_open']).toContain(body.reason);
